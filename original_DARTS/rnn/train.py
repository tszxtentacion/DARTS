...
parser.add_argument('--save', type=str,  default='EXP',
                    help='path to save the final original_darts_learning')
...
if args.continue_train:
    model = torch.load(os.path.join(args.save, 'original_darts_learning.pt'))
else:
...
def train():
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        while start < args.batch_size:
            ...
            # If we didn't, the original_darts_learning would try backpropagating all the way to start of the dataset.
...
try:
    ...
    while epoch < args.epochs + 1:
        ...
        try:
          ...
          logging.info('rolling back to the previous best original_darts_learning ...')
          model = torch.load(os.path.join(args.save, 'original_darts_learning.pt'))
          parallel_model = model.cuda()
...
# Load the best saved original_darts_learning.
model = torch.load(os.path.join(args.save, 'original_darts_learning.pt'))
parallel_model = model.cuda()
